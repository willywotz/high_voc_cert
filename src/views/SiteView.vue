...
template(v-if="user.isAdmin")
  form.border.rounded.bg-white.overflow-hidden(@submit.prevent="updateSite")
    .flex.flex-col.px-4
      ...
      label.mb-2(for="site-title") ชื่อเว็บ :
      input.mb-4.border.rounded.py-1.px-2(type="text" name="title" id="site-title" v-model="site.title")
    .bg-gray-50.py-2.px-4.flex.items-baseline
      .flex-grow
      button.py-1.px-2.border.rounded.bg-white(type="submit") ปรับปรุง
  ...
  .bg-white.divide-y(class="sm:border sm:rounded")
    ...
    .flex.divide-x(v-for="(user, index) in users" :key="user.id")
      ...
      button.py-2.px-4(class="hover:bg-gray-50" @click.prevent="toggleAdminUser(index)") {{ !user?.isAdmin ? 'ตั้งผู้ดูแล' : 'ยกเลิกผู้ดูแล' }}
      button.py-2.px-4(class="hover:bg-gray-50" @click.prevent="toggleBanUser(index)") {{ !user?.banned ? 'แบน' : 'ยกเลิกแบน' }}
    .p-4(v-if="!exams") ไม่มีข้อสอบในระบบ
</template>
